...
def test_valid_material():
    instance, errors = XmiStructuralMaterial.from_dict(input_data.valid_material_input)
...
def test_missing_material_type():
    instance, errors = XmiStructuralMaterial.from_dict(input_data.missing_material_type_input)
    assert instance is None
    assert any("material_type" in str(e) for e in errors)


def test_invalid_material_type():
    instance, errors = XmiStructuralMaterial.from_dict(input_data.invalid_material_type_input)
    assert instance is None
    assert any("material_type" in str(e) for e in errors)


# .venv/bin/python -m pytest tests/xmi/v2/test_entities/test_xmi_structural_material.py