...
from tests.xmi.v2.test_inputs import xmi_unit_input as input_data


def test_valid_unit():
    instance = XmiUnit(**input_data.valid_unit_input)

    assert instance is not None
    assert instance.entity == "Beam"
    assert instance.attribute == "Length"
    assert instance.unit.value == "m"
    assert instance.entity_type == "XmiUnit"
    assert UUID(instance.id)
    assert instance.name.startswith("XmiUnit_")


def test_missing_entity():
    with pytest.raises(ValueError) as exc_info:
        XmiUnit(**input_data.missing_entity_input)
    assert "Entity" in str(exc_info.value)


def test_invalid_unit():
    with pytest.raises(ValueError) as exc_info:
        XmiUnit(**input_data.invalid_unit_input)
    assert "Input should be" in str(exc_info.value)

def test_explicit_id_and_name():
    instance = XmiUnit(**input_data.explicit_id_and_name_input)
...
# .venv/bin/python -m pytest tests/xmi/v2/test_entities/test_xmi_unit.py